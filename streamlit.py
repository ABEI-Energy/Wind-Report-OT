...
with colpr1:
    stDict['territorial1Key'] = df_countries.loc[df_countries['Country']==country,'territorial1Key'].unique()
    df_country = countryDf(f'../resources/Countries/{country}.csv')
...
with colpr2:
    stDict['territorial2Key'] = df_countries.loc[df_countries['Country']==country,'territorial2Key'].unique()
    if territorial1:
        territorial2Flag = False
        territorial2 = df_country['territorial2Key'].unique()
        stDict['territorial2'] = st.selectbox(stDict['territorial2Key'], territorial2, disabled = territorial2Flag)

with colpr3:
    stDict['territorial3Key'] = df_countries.loc[df_countries['Country']==country, 'territorial3Key'].unique()
    if territorial1:
        territorial2Flag = False
        territorial3 = df_country['territorial3Key'].unique()
...
if flagDone:
    ...
    stDict['modelWind'] = st.selectbox('Years of the historical data', ('MERRA-2', 'ERA-5', 'EMD Global Wind Data'))
